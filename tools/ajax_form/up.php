...
if ($_GET['action'] == 'save') {
    ...
    $up = new upload();
    $up->set_dir(dirname(__FILE__).'/uploads/','{y}-{m}-{d}');
    $up->set_thumb(100,80);
   // $up->set_watermark(dirname(__FILE__).'/jblog/images/watermark.png',7,90);
    $fs = $up->execute();
...
class upload {
    ...
    function upload($types = 'jpg|png|gif|bmp|jpeg', $maxsize = 1024, $field = 'attach', $time = '') {
    ...
    function execute() {
        ...
        foreach ($keys as $key) {
			...
			$files[$key]['url'] = 'http://'.$_SERVER['HTTP_HOST'].'/'.str_replace("\\","/",str_replace(realpath($_SERVER['DOCUMENT_ROOT']),'',realpath($filedir))).'/'.$filename;
