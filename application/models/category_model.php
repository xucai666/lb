...
/**
 ...
 class Category_model  extends CI_Model{
	...
	function __construct(){
		...
		parent::__construct();		
		$this->table  = $this->mydb->table('category');
	...
	public function fetch_category($sn=null){
		...
		return $this->myform->array_re_index($category,'c_sn','c_title');
	...
	function fetch_parent($sub){
		...
		return  $this->mydb->fetch_value($sql);
	...
	function fetch_new_sn($cur_sn,$parent){
		if(strcmp(substr($cur_sn,0,-2),$parent)!=0){
			...
			$new_c_sn =  $this->mydb->fetch_value($sql,'new_c_sn');
