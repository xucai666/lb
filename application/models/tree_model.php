...
class Tree_model extends CI_Model{
	...
	function fetch_select($treeId=null,$v_field='name'){
		...
		return $this->myform->array_re_index($this->db->select("id,name as init_name",false)->select('LEVEL,CONCAT(REPEAT("|-",LEVEL),NAME) as name',false)->from('tree_node')->where('treeId',$treeId)->order_by('leftId','asc')->get()->result_array(),'id',$v_field);
	}
	...
	function fetch_select_query($treeId=null){
		$treeId = $treeId?$treeId:$this->get_root();
		$ls =  $this->myform->array_re_index($this->db->select("id,name as init_name,leftId,rightId",false)->select('LEVEL,CONCAT(REPEAT("|-",LEVEL),NAME) as name',false)->from('tree_node')->where('treeId',$treeId)->order_by('leftId','asc')->get()->result_array(),'id');
		$ls_new = array();
		foreach($ls as $v){
