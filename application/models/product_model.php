...
class Product_model extends CI_Model{
	...
	function db_config(){
		return array(
			'main'=>array(
				'table_name'=>$this->mydb->table('products'),
				'primary_key'=>'pro_id',
			)
		);
	}
	...
	function db_config_order(){
		return array(
			...
			'detail'=>array(
				'table_name'=>$this->mydb->table('order_detail'),
	...
	function product_left(){
		...
		$sql  = "select * from ".$this->mydb->table("products")." order by pro_id desc limit 8 ";
	...
	function product_class_left(){
		...
		$sql  = "select * from ".$this->mydb->table("category")." where c_sn like '".$this->c_sn."%' order by c_sn asc,c_level asc ";
		$list = $this->db->query($sql)->result_array();
		return $list;	
	...
	function product_select(){
		...
		$sql  = "select * from ".$this->mydb->table("products")." order by pro_id desc";
		$list = $this->db->query($sql)->result_array();
		$list = $this->myform->array_re_index($list,'pro_id','pro_title');
		return $list;	
