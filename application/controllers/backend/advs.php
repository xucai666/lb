...
class Advs extends CI_Controller{
	...
	function action_add(){
		...
		if($adv_id){
			...
			$detail = $this->db->query("select * from ".$this->mydb->table('adv_detail')." where adv_id=".$adv_id)->result_array();
		}else{
	 ...
	 function action_save(){
	 	try{		 	
		 	...
		 	if($this->form_validation->run()==true){
		 		...
		 		$this->mypage->pop_redirect('已保存',site_url('backend/advs/action_list/?parent_cat='.$main['info_class_sn']));
		 	}else{
		 		;
				$data['editor']  = $this->im->editor($main['info_content']);
		 		$this->mypage->load_backend_view('advs_add',$data);
		 	}
	 ...
	 */
	 function action_list(){
	 	try{
	 		...
	 		$this->mypage->backend_redirect('advs/action_list/',$e->getMessage());
	 	}			
	 }
	 ...
	 */
	 function action_del(){
	 	try{
			$this->mydb->delete($this->input->get('adv_id'),$this->im->db_config());
			$this->mypage->pop_redirect('已删除',site_url('backend/advs/action_list/'));
