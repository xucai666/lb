...
var FormValid = function(frm) {
    ...
    this.passed = function() {
        if (this.errMsg.length > 0) {
            FormValid.showError(this.errMsg,this.errName,this.frm.name,this.errId);
    ...
    this.addErrorMsg = function(name,str,id) {
...
//display error 
FormValid.showError = function(errMsg,errName,frmName,errId) {
	//清空全部存在的提示span
	var i;
	for(i in FormValid.allName){
		 	if(document.getElementById("error_span_"+i)!=undefined){
		 		removeElement(document.getElementById("error_span_"+i));
		 	}
	}
	//为错误输入增加提示文字
	for (key in errMsg) {
			e = document.createElement('span');
			e.id = "error_span_"+key;
			e.innerText = errMsg[key];
			e.style.display='block';
			insertAfter(e,document.getElementById(""+errId[key]+""));
			
	 }
	 //为提示框创建显示样式
	if(errMsg.length>0)	displayError("[id^='error_span_']");
}
...
function validator(frm) {
	...
	for (var i=0; i<formElements.length;i++) {
		...
		if (msgs.length>0) {
			for (n in msgs) {
				fv.addErrorMsg(formElements[i].name,msgs[n],formElements[i].id);
			}
		}
	}
	return fv.passed();
