...
class FCKeditor
{
	 ...
	 * @return string
	 */
	public function CreateHtml($config=null)
	{	
		$this->BasePath		= base_url().'/fckeditor/' ;
		$this->Config		= array_slice($config,5) ;				
		if($config){
			$this->InstanceName			= empty($config['i'])?"Default":$config['i'] ;
			$this->Width				= $config['w'] ;
			$this->Height				=  $config['h'] ;
			$this->ToolbarSet			= empty($config['t'])?"Basic":$config['t'] ;
			$this->Value				= $config['v'] ;
