 ...
 class Init_auth{
        ...
        $CI = &get_instance();
 		...
 		if($auth):
		    ...
		    $righs = $this->fetch_auth('user_rights');
	   		...
	   		if(!$flag){
						$CI->init_page->backend_redirect('javascript:history.back(1);','抱歉，无此权限');	
			}
	 ...
	 */
 	...
 	function getAuthMenu(){
 		$CI = &get_instance();
 		if($CI->init_cache->cache_exists('admin_rights_config',lang_get())){
 			$data = $CI->init_cache->cache_fetch('admin_rights_config',null,lang_get());
 		}else{
